...
const energyConfig = {
  high: { icon: Zap, label: "High Energy", color: "bg-rose-500/10 text-rose-600 border-rose-200" },
  medium: { icon: Coffee, label: "Medium Energy", color: "bg-amber-500/10 text-amber-600 border-amber-200" },
  low: { icon: Moon, label: "Low Energy", color: "bg-emerald-500/10 text-emerald-600 border-emerald-200" },
}

const priorityConfig = {
  urgent: { icon: AlertCircle, label: "Urgent", color: "bg-red-500/10 text-red-600" },
  important: { icon: Clock, label: "Important", color: "bg-blue-500/10 text-blue-600" },
  normal: { icon: Check, label: "Normal", color: "bg-muted text-muted-foreground" },
}
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="text-center space-y-2">
        ...
        <h1 className="text-3xl font-semibold tracking-tight">Review Your Tasks</h1>
                          ...
                          <SelectContent>
                            {DURATION_OPTIONS.map((mins) => (
                              <SelectItem key={mins} value={mins.toString()}>
                                {mins >= 60
                                  ? `${mins / 60}${mins % 60 === 0 ? "" : `.${(mins % 60) / 6}`} hr`
                                  : `${mins} min`}
                              </SelectItem>
                            ))}
                          </SelectContent>
