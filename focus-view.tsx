...
export function FocusView({
  ...
  const [selectedTimer, setSelectedTimer] = useState<number>(task?.duration || 40)
  ...
  useEffect(() => {
    ...
    setNotes("")
    setShowNotes(false)
    setSelectedTimer(task?.duration || 40)
      ...
      <Card className="border-2 border-primary/20 bg-card shadow-lg">
        <CardContent className="p-8">
          <div className="text-center">
            ...
            <h2 className="text-2xl md:text-3xl font-semibold mb-4 text-balance leading-snug">{task.title}</h2>
